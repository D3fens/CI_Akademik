...
class Web_App_Model extends CI_Model {
	...
	{
		$kon = mysqli_connect('localhost', 'root', '', 'krs');
		$u = mysqli_real_escape_string($kon, $usr);
		$p = md5(mysqli_real_escape_string($kon, $psw));
	...
	{
		return $this->db->query('SELECT 
		a.kd_jadwal,e.kode_jur,f.nama_kampus,b.nama_mk,b.kd_mk,b.semester,b.jum_sks,a.kapasitas,a.status_ajar,a.kelas,c.kd_dosen,a.jadwal,d.Peserta,d.CalonPeserta,c.nama_dosen FROM 
		`tbl_jadwal` a 
		left join tbl_mk b on a.kd_mk=b.kd_mk 
		left join tbl_dosen c on a.kd_dosen=c.kd_dosen 
		left join (
		...
		on a.kd_jadwal=d.kd_jadwal
		left join tbl_prodi e on a.kode_jur=e.kode_jur
		left join tbl_kampus f on a.kd_kampus=f.kd_kampus
		')
		;
	}
	...
	{
		return $this->db->query("SELECT a.kd_jadwal, a.kode_jur, a.kd_kampus, a.jadwal,  b.kd_mk, b.nama_mk, c.kd_dosen, c.nama_dosen, d.kd_tahun, a.kapasitas, a.kelas_program, a.status_ajar, 
		a.kelas FROM tbl_jadwal a 
		left join tbl_mk b on a.kd_mk=b.kd_mk
		left join tbl_dosen c on a.kd_dosen=c.kd_dosen
		left join tbl_thn_ajaran d on a.kd_tahun=d.kd_tahun
		left join tbl_prodi e on a.kode_jur=e.kode_jur
		left join tbl_kampus f on a.kd_kampus=f.kd_kampus
		where a.kd_jadwal='".$kd_jadwal."'");
	}
